...
/**
 * Wcześniejsza z dat: dataWaznosci oraz (dataOtwarcia + PAO miesięcy), jeśli produkt otwarty.
 * Null, gdy żadna z dat nie jest znana.
 */
fun ProduktEntity.terminEfektywny(): LocalDate? {
    val terminPoOtwarciu = if (status == StatusProduktu.OTWARTE && dataOtwarcia != null && okresZuzyciaPoOtwarciuMiesiace != null) {
        dataOtwarcia.plusMonths(okresZuzyciaPoOtwarciuMiesiace.toLong())
    } else {
        null
    }
